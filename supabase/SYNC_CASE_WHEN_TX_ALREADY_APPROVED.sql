-- Si el admin aprueba el pago ANTES de que el cliente cree el caso, el UPDATE en
-- transactions ya pasó y el caso queda en awaiting_payment sin promoción.
-- BEFORE INSERT: si la transacción vinculada ya está approved, pasa a pending_approval.
...
create trigger tr_cases_sync_tx_on_insert
  ...
  for each row
  execute function public.cases_sync_with_transaction_on_insert();

-- Casos ya guardados que quedaron atascados (pago approved, caso awaiting_payment)
with upd as (
